...
const fadeMeshes = (group: THREE.Group | null, opacity: number) => {
  ...
  group.traverse((child: THREE.Object3D) => {
    if ((child as THREE.Mesh).isMesh) {
      ...
      // Normalize materials: always treat as an array
      ...
      materials.forEach((mat) => {
        ...
        gsap.to(mat, { opacity, duration: ANIMATION_DURATTION });
      });
    }
  });
};
...
const moveGroup = (group: THREE.Group | null, x: number) => {
  ...
  gsap.to(group.position, { x, duration: ANIMATION_DURATTION });
};

const ModelSwitcher = ({ isMobile, scale }: Props) => {
